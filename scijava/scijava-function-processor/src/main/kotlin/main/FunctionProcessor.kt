...
import templates.Computers
import templates.Consumers
import templates.Functions
import templates.Inplaces
import java.io.File
...
class FunctionProcessor(private val codeGenerator: CodeGenerator,
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        if (!processed) {
            ...
            Computers(javaDir).build()
            Consumers(javaDir).build()
            Functions(javaDir).build()
            Inplaces(javaDir).build()
        }
        return emptyList()
    }
}